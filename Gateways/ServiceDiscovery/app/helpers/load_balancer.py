...
class LoadBalancer:
    ...
    def get_next_host(self, service_type):
        service_type = manager_pb2.ServiceType.Name(service_type)
        try:
            if service_type not in self.services:
                print(f"LB (SERVICE TYPE NOT FOUND): Service type `{service_type}` not found in services.")
                return None

            hosts = self.services[service_type]

            if not hosts:
                print(f"LB (HOSTS NOT FOUND): No hosts found for service type `{service_type}`.")

            index = self.service_index.get(service_type, 0)

            if index >= len(hosts):
                index = 0  # Reset the index if it exceeds the number of hosts available

            next_host = hosts[index]
            self.service_index[service_type] = (index + 1) % len(hosts)
